...
def get_task(request):
	if request.method == 'GET':
		...
		if(machine.macid == -1 or machine.projectid == -1):
			...
			cmd = 'task|'+str(project.id)+'|'+str(machine.macid)+'|'+str(project.totalmachines)+'|'+str(machine.countnow)
...
def handle_crash(id):
	crash = crashs.objects.get(id=id)
	mutex.acquire()
	try:
		try:
			freemachine = machines.objects.filter(status='free').order_by('ip')[0:1]
			freemachine.macid = crash.macid
